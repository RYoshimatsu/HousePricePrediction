...
SV regression finds a hyperplane that maximizes the number of observations within the margin. epsilon is the margin of error and cost is the multiplicative factor for the slack.
...
tuneResult <- tune(svm, log(price) ~ . , data=train, ranges=list(epsilon=seq(0,1,0.1), cost=2^(2:9)))
print(tuneResult)
plot(tuneResult)
```

```{r}
# Perform another grid search (narrower range)
tuneResult <- tune(svm, log(price) ~ . , data=train, ranges=list(epsilon=seq(0,0.2,0.02), cost=2^(-2:2)))
print(tuneResult)
plot(tuneResult)
```
...
sv_regression <- tuneResult$best.model

# Train
predictions.train <- predict(sv_regression, train)
rmse.train <- sqrt(mean((exp(predictions.train) - train$price)^2))
sse.train <- sum((exp(predictions.train) - train$price)^2)
sst.train <- sum((train$price - mean(train$price))^2)
r_square.train <- 1 - sse.train/sst.train
cat('Support Vector Regression on Train (RMSE & R Square) :\t', rmse.train, '\t', r_square.train, '\n')

# Test
predictions.test <- predict(sv_regression, test)
rmse.test <- sqrt(mean((exp(predictions.test) - test$price)^2))
sse.test <- sum((exp(predictions.test) - test$price)^2)
sst.test <- sum((test$price - mean(test$price))^2)
r_square.test <- 1 - sse.test/sst.test
cat('Support Vector Regression on Test (RMSE & R Square) :\t', rmse.test, '\t', r_square.test, '\n')

p1 <- ggplot(aes(x=price, y=exp(predictions.train)), data=train) + geom_point(alpha=0.5) + geom_abline(intercept=0, slope=1, color='blue') + xlab("Actual Value") + ylab("Predited Value") + ggtitle("SVR on Train Data")
p2 <- ggplot(aes(x=price, y=exp(predictions.test)), data=test) + geom_point(alpha=0.5) + geom_abline(intercept=0, slope=1, color='blue') + xlab("Actual Value") + ylab("Predited Value") + ggtitle("SVR on Test Data")
...
cat('sv regression:\t\t')
eval_metrics_others(sv_regression, validation)
cat('regression tree:\t')
